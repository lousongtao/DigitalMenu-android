        ...
        thCategory1.setOnTabChangedListener(new TabHost.OnTabChangeListener() {
            ...
            public void onTabChanged(String tabId) {
                for (int i = 0; i < thCategory1.getTabWidget().getChildCount(); i++) {
                    thCategory1.getTabWidget().getChildAt(i).setBackgroundResource(R.color.colorCategory1Unselected);
                }
                thCategory1.getTabWidget().getChildAt(thCategory1.getCurrentTab()).setBackgroundResource(R.color.colorCategory1Selected);
            }
        });

        //set the first tab as selected and change background color
        thCategory1.setCurrentTab(0);
        thCategory1.getTabWidget().getChildAt(0).setBackgroundResource(R.color.colorCategory1Selected);
    }
        ...
        th.setOnTabChangedListener(new TabHost.OnTabChangeListener() {
            ...
            public void onTabChanged(String tabId) {
                for (int i = 0; i < th.getTabWidget().getChildCount(); i++) {
                    th.getTabWidget().getChildAt(i).setBackgroundResource(R.color.colorCategory2Unselected);
                }
                th.getTabWidget().getChildAt(th.getCurrentTab()).setBackgroundResource(R.color.colorCategory2Selected);
            }
        });

        //set the first tab as selected and change background color
        th.setCurrentTab(0);
        th.getTabWidget().getChildAt(0).setBackgroundResource(R.color.colorCategory2Selected);
        return th;
    }
...
//        List<Dish> dishes = mainActivity.getDbOperator().queryDishByParentId(c2.getId());
        ...
        for(int i = 0; i< dishes.size(); i++){
            ...
            if (dish.getPictureName() != null) {
                fc.setBackground(IOOperator.getDishImageDrawable(mainActivity.getResources(), InstantValue.LOCAL_CATALOG_DISH_PICTURE + dish.getPictureName()));
            }
